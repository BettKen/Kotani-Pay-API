...
const {
  getUserId, getTargetCountry, checkIfSenderExists, checkisUserKyced, addUserKycToDB,
} = require('../../modules/libraries');
const lib = require('../../modules/libraries');
const {
  getPinFromUser, createcypher, sendMessage, isValidPhoneNumber, validateMSISDN,
} = require('../../modules/utilities');
// GLOBAL ENV VARIABLES
const { iv } = require('../contants');

// 👍🏽
exports.kycUserUpdate = async (req, res) => {
  try {
...
// 👍🏽
exports.kycUserActivate = async (req, res) => {
  try {
...
// 👍🏽
exports.kycUserCreate = async (req, res) => {
  console.log(`Received request for: ${req.url}`);
  try {
    const { phoneNumber } = req.body;
...
// 👍🏽
exports.kycUserIsVerifiedCheck = async (req, res) => {
  console.log(`Received request for: ${req.url}`);
  try {
    const { phoneNumber } = req.body;
    const { permissionLevel } = req.user;
...
exports.kycUserGetDetailsByPhone = async (req, res) => {
  try {
    const { phoneNumber } = req.body;
    const { permissionLevel } = req.user;
    const targetCountry = getTargetCountry(permissionLevel, req.user.targetCountry);
...
// 👍🏽
exports.kycUserSetDetails = async (req, res) => {
  try {
    console.log(`Received request for: ${req.url}`);
    const { phoneNumber } = req.body;
    const { permissionLevel } = req.user;
...
exports.programsKycUpdateUser = async (req, res) => {
  try {
    console.log(`Received request for: ${req.url}`);
    const { phoneNumber } = req.body;
