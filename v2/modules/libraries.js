...
const { iv,  phoneHashfn, USD_TO_KES } = require('../src/contants/index');

// ENV VARIABLES
// const iv = functions.config().env.crypto_iv.key;
// const  phoneHashfn = functions.config().env.algo.msisdn_hash;
const {
  getPublicAddress, generatePrivKey, weiToDecimal, sendcUSD, getContractKit,
} = require('./celokit');
...
exports.signupDeposit = async (publicAddress) => {
  ...
  const escrowPrivkey = await getSenderPrivateKey(escrowInfo.data().seedKey, escrowMSISDN, iv);

  const receipt = await sendcUSD(escrowInfo.data().publicAddress, publicAddress, '0.01', escrowPrivkey);
...
const getUserId = (senderMSISDN) => new Promise((resolve) => {
  const senderId = crypto.createHash( phoneHashfn).update(senderMSISDN).digest('hex');
  resolve(senderId);
});
...
exports.createNewUser = (userId, userMSISDN) => new Promise((resolve) => {
...
exports.verifyNewUser = async (userId, email, newUserPin, firstname, lastname, idnumber, dateofbirth, userMSISDN) => new Promise((resolve) => {
...
exports.getTargetCountry = (permissionLevel, targetCountry) => {
  ...
  _targetCountry = 'KE'; return _targetCountry;
};
